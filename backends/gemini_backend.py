...
----------------------------
Implementasi TableauAgentSession berbasis Gemini (google-genai) + function
calling ke MCP. Jangan import file ini langsung — akses lewat agent_service.py
yang memilih backend berdasarkan LLM_PROVIDER di .env.
"""
...
def _mcp_tools_to_gemini_tools(mcp_tools: list) -> list[genai_types.Tool]:
    declarations = [
...
class TableauAgentSession:
    """
    Satu instance = satu percakapan (histori chat + satu subprocess MCP
    server + satu koneksi Tableau yang sudah sign-in).
    """
    ...
    async def connect(self) -> list[str]:
        ...
        self.gemini_tools = _mcp_tools_to_gemini_tools(tools_response.tools)
    ...
    async def ask_stream(
        ...
        try:
            for _ in range(MAX_TOOL_LOOPS):
                ...
                self.chat_history.append(genai_types.Content(role="user", parts=response_parts))
